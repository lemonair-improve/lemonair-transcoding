...
public class AwsService {
	 ...
	 * @return s3 업로드 후 업로드 된 url
	 */
	public String uploadToS3Async(String filePath) {
		...
		return upload(putObjectRequest, filePath, key);
	}
	 ...
	 * @return 업로드 성공 후 S3에 업로드된 url
	 */
	private String upload(PutObjectRequest putObjectRequest, String filePath, String key) {
		Path path = Path.of(filePath);
		// TODO: 2023-12-20 현재 업로드된 영상의 url을 리턴하고 로깅하는 로직 때문에 join() 메서드로 블락이 걸려있음
		amazonS3AsyncClient.putObject(putObjectRequest, path).join();
		String uploadedUrl = amazonS3SyncClient.getUrl(bucket, key).toString();
